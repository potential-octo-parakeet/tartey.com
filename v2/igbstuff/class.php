<?php
	class newsfeed extends db{
		function wall_has_comment($msg_id){
			$this->connect();
			$sql = sprintf("SELECT 1 FROM wall_message_comment WHERE msg_id='%d' 
			UNION SELECT 1 FROM wall_message_cool  WHERE msg_id='%d'",$msg_id,$msg_id);
			$sql = mysql_query($sql);
		...
		function wall_message_comment($wall_id,$limit=10){
			$this->connect();
			$sql = sprintf("SELECT user.id,user.name,user.picture50,wall_message_comment.`comment`,
		...
		function wall_message($offset){
			$this->connect();
			$sql = sprintf("SELECT user.id AS 'uid',user.name,user.picture50,wall_message.id,wall_message.context,
						CONVERT_TZ(wall_message.`timestamp`,'-07:00','+06:00') AS 'date' 
						FROM wall_message JOIN user ON wall_message.from_userid=user.id ORDER BY wall_message.id DESC limit %d,25",$offset);
			$sql=mysql_query($sql);
			$a=array();$i=0;
			while($row=mysql_fetch_assoc($sql)){
				$a[$i]['wall_id']	= $row['id'];
	...
	class igobig extends db{
		...
		function get_messages($id){
			$this->connect();
			$sql=sprintf("SELECT user_message.id,user.picture50,user.name,user_message.from_userid,user_message.`text`,user_message.token,
		...
		function friends($id,$offset=0,$limit=10){
			$this->connect();
			$sql=sprintf("SELECT user.id,user.name,user.picture50 FROM user JOIN friends ON user.id=friends.friends_id WHERE friends.user_id='%d'
			 ORDER BY name ASC LIMIT %d,%d",$id,$offset,$limit);
			$sql=mysql_query($sql);
			$a=array();$i=0;
			while($row=mysql_fetch_assoc($sql)){
				$a[$i]['id']	= $row['id'];
		...
		function search_people($q,$offset=0,$limit=8){
			$this->connect();
			$sql=sprintf("SELECT id,name,picture50 FROM user WHERE name LIKE '%s%%' LIMIT %d,%d",mysql_real_escape_string($q),$offset,$limit);
			$sql=mysql_query($sql);
			$a=array();$i=0;
			while($row=mysql_fetch_assoc($sql)){
				$a[$i]['id']	= $row['id'];
		...
		function get_user_picture_small($email){
			$this->connect();
			$sql=sprintf("SELECT picture50 FROM user WHERE email='%s'",$email);
			$sql=mysql_query($sql);
			return mysql_result($sql,0);
		}
	...
	class notifications extends db{
		function friendrequest($id,$offset=0,$limit=8){
			$this->connect();
			$sql=sprintf("SELECT user.id,user.name,user.picture50 FROM user JOIN friendrequest ON user.id=friendrequest.from_userid WHERE friendrequest.to_userid='%d' LIMIT %d,%d",$id,$offset,$limit);
			$sql=mysql_query($sql);
			$a=array();$i=0;
			while($row=mysql_fetch_assoc($sql)){
				$a[$i]['id']	= $row['id'];
		...
		function friendRequestCounter($id){
			$this->connect();
			$sql=sprintf("SELECT user.id,user.name,user.picture50 FROM user JOIN friendrequest ON user.id=friendrequest.from_userid WHERE friendrequest.to_userid='%d'",$id);
			$sql=mysql_query($sql);
			$i=0;
			while($row=mysql_fetch_assoc($sql)){
				$i++;
	...
	class db{
		function connect(){
			if($_SERVER['HTTP_HOST']=="x.igobig.org"){
				mysql_connect("localhost","root","");mysql_select_db("igobig");
			}elseif($_SERVER['HTTP_HOST']=="tartey.com"){
				mysql_connect("tartey.db.8603859.hostedresource.com","tartey","BOHOLt@rsi3r");mysql_select_db("tartey");
			}else{
				mysql_connect("localhost","igobigc1_user","qWho(+,Bqp]f");mysql_select_db("igobigc1_adsteknik");
			}
		}
	}
?>