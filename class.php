<?php
	...
	class cejas extends db{
		function chpwd($email,$newpwd){
			$this->connect();
			$tkn = md5(rand(0,9999999)+time());
			$sql = sprintf("UPDATE user SET password='%s',token='%s' WHERE email='%s'",md5($newpwd),$tkn,mysql_real_escape_string($email));
			mysql_query($sql);
		}
		function tokenize($email){
			$this->connect();
			$tkn = md5(rand(0,9999999)+time());
			$sql = sprintf("UPDATE user SET token='%s' WHERE email='%s'",$tkn,mysql_real_escape_string($email));
			mysql_query($sql);
		}
		function me($email,$password){
			$this->connect();
			$sql = sprintf("SELECT * FROM user WHERE email='%s' AND password='%s'",mysql_real_escape_string($email),md5($password));
			$sql = mysql_query($sql);
			return mysql_fetch_assoc($sql);
		}
		function user_basic_info($id){
			$this->connect();
			$sql=sprintf("SELECT *,DATE_FORMAT(birthday,'%%M %%d, %%Y') as birthday FROM user WHERE id='%d'",$id);
			$sql=mysql_query($sql);
			return mysql_fetch_assoc($sql);
		}
		function forgot_password($email){
			$this->connect();
			$sql = sprintf("SELECT * FROM user WHERE email='%s'",mysql_real_escape_string($email));
			$sql = mysql_query($sql);
			return mysql_fetch_assoc($sql);
		}
		...
		function friends($id,$offset=0,$limit=10){
			$this->connect();
			$sql=sprintf("SELECT user.id,user.name,user.img50 FROM user JOIN friends ON user.id=friends.friends_id WHERE friends.user_id='%d'
